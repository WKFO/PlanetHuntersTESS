...
def process_lc(lcfile, tics_inj, args):
    ...
    injected_pl = glob("/Users/Nora/Documents/research/TESS/ETE-6/ground_truth/Injected/Planets/Planets_*{:d}.txt".format(tics_inj))
    ...
    l_pl1 = np.where((pl['col1'] == tics_inj) * (pl['col16'] >= args.min_snr))[0]        
    l_pl2 = np.where((pl['col1'] == tics_inj) * (pl['col16'] < args.min_snr))[0]        
    ...
    for j in range(args.chunks):
        ...
        if nm > 0: # only make figures and save files if there are transits in that chunk 
            ...
            injtrans_pl, injtrans_eb, tic = transit_loc.transit_loc_single(tics_inj, args.min_snr, xmin, xmax)
                ...
                if trans_loc == None: # if no transits exist in that quarter
                    ...
                    for l,t in enumerate(transit_list):
                        ...
                        if t != '':
                            ...
                            feedback_tolerance[l].append(transit_duration/24*1.2)
                        else:
...
if __name__ == '__main__':
    ...
    if mpi_rank == mpi_root:
        ...
        # select the TICs of the injected planets
        ...
        lcfile0 = sorted(glob('{:s}/tess*lc.fits'.format(args.lcdir)))  # will later want to filer out the ones where we know there are planets.
        random.seed(seed_num)  
        ...
        # make the header names for the feedback.
        ...
        if (not with_mpi) or (mpi_size==1) or (nlc==1):
            ...
            for i,f in enumerate(lcfile0):
                ...
                res = process_lc(f, tics_inj[i], args)
